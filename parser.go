...
func addQueueLink(httplink, action, referer string, depth int, wg *sync.WaitGroup) {
	...
	threads <- 1 // will block if there is MAX ints in theads

	wg.Add(1)
	defer wg.Done()

	// ensure only one process and read/write to processed map
	mapMutex.Lock()
	...
	<-threads // remove an int from threads, allowing another to proceed
}
