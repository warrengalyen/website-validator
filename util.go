...
func head(httplink string) {
	// check it's not a host that won't work well, else ignore
	...
	// some host block HEAD requests, so we do a standard GET instead
...
func getResponse(httplink string) {
	// completely ignore requests to ignoreHosts
	if ignoreHosts.MatchString(httplink) {
		return
	}
